...
export function formatOrder(row: any, items: any[] = [], statusHistory: any[] = []) {
  return {
    ...
    deliveryDistanceKm: row.delivery_distance_km !== null && row.delivery_distance_km !== undefined ? Number(row.delivery_distance_km) : null,
    deliveryLatitude: row.delivery_latitude !== null && row.delivery_latitude !== undefined ? Number(row.delivery_latitude) : null,
    deliveryLongitude: row.delivery_longitude !== null && row.delivery_longitude !== undefined ? Number(row.delivery_longitude) : null,
